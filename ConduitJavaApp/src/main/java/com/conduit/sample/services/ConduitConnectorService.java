...
public class ConduitConnectorService<T, T1, T2> {
    ...
    public <T> T createConnector() {
        ...
        try {
            return (T) client.post(createConnectorRequest, "https://localhost/api/metadata/datasources", createConnectorResponse);
        } catch (Exception e) {
    ...
    public boolean verifyIsUniqueConnectorName(String name) {
        ...
        try {
            List<String> response = (List<String>) client.get(null, "https://localhost/api/metadata/datasourcesNames", existingConectorResponse);
