...
public class ConduitRESTQueryService<T> {
    ...
    public void verifyExistingConnectors(String connectorName) {
        ...
        try {
            QueryMetadataResponse response = ((QueryMetadataResponse) client.get(null, "https://localhost/api/query/metadata/all", queryMetadataResponse));
    ...
    public T executeQuery(String sql) {
        ...
        try {
            ...
            return client.post(executeQueryRequest, "https://localhost/api/query/execute", executeQueryResponse);
        } catch (Exception e) {
